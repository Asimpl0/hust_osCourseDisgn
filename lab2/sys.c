...
SYSCALL_DEFINE2(copy, char*, s1, char*, s2)
{
	//打开待读取文件
    mm_segment_t old_fs = get_fs(); // 保存原来的段
    set_fs(KERNEL_DS); // 设置为数据段
    int f_read = sys_open(s1, O_RDONLY,0);
    if (f_read == -1)
    {
    ...
    //打开待写入文件
    int f_write = sys_open(s2, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (f_write == -1)
    {
    ...
    char buf[10];
    int num;
    while ((num = sys_read(f_read, buf, 10)) > 0)
    {
